...
func showMovieAdda(w fyne.Window) {
	...
	content := container.NewVBox(input, widget.NewButton("SEARCH", func() {
		...
		label1 := canvas.NewText(fmt.Sprintf("Movie Name : %s", Welcome.Results[0].Title), color.White)
		label2 := canvas.NewText(fmt.Sprintf("Release Date : %s", Welcome.Results[0].ReleaseDate), color.White)
		label3 := canvas.NewText(fmt.Sprintf("Popularity : %.2f", Welcome.Results[0].Popularity), color.White)
		label4 := canvas.NewText(fmt.Sprintf("Original Language : %s", Welcome.Results[0].OriginalLanguage), color.White)
		label5 := canvas.NewText(fmt.Sprintf("Movie ID : %d", Welcome.Results[0].ID), color.White)
		label6 := canvas.NewText(fmt.Sprintf("Ratings : %.2f", Welcome.Results[0].VoteAverage), color.White)
